...
`drova_web` — отдельный сервис, который запускает воркеры как дочерние процессы и предоставляет браузерный UI для их управления. Запускается из той же конфигурации, что и multi-host `drova_poll`.
...
Браузер запросит Basic Auth (логин/пароль из `DROVA_WEB_USER` / `DROVA_WEB_PASSWORD`).

---

#### Таблица хостов — описание колонок

UI отображает таблицу со следующими колонками:

| Колонка | Что показывает |
|---|---|
| **Хост** | IP-адрес или имя Windows ПК |
| **Воркер** | Статус дочернего процесса: `Работает` / `Остановлен` / `Отключён` / `Ошибка` |
| **SSH** | Результат последнего SSH-зонда: ✓ = доступен и пароль верный; ✗ = недоступен или неверный пароль; ? = ещё не проверялся |
| **SD** | Shadow Defender: 🔒 = хотя бы один диск в теневом режиме; 🔓 = теневой режим не активен; ? = нет данных |
| **Огр.** | Ограничения реестра: ✓ = все ключи присутствуют; ✗ красный = SD активен, но ключей нет (проблема!); ✗ жёлтый = SD не активен, ключей нет (ожидаемо для простаивающего ПК); ? = нет данных |
| **Сессия** | Текущий Drova-сеанс: 🖥 = активный рабочий стол; ▷ = активный сеанс, но не рабочий стол; ○ = нет сеанса; ? = нет данных |
| **Диагн.↑** | Результат стартовой диагностики (см. ниже) |
| **PID** | PID дочернего процесса воркера |
| **Действия** | Кнопки управления (см. ниже) |

Наведите курсор на иконку SSH/SD/Огр./Сессия — покажется tooltip с временем последней проверки. Таблица обновляется автоматически каждые 3 секунды.

---

#### Колонка «Диагн.↑» — стартовая диагностика

При каждом запуске воркера `GamePCDiagnostic` входит в Shadow Defender, применяет все реестровые патчи, проверяет их, выходит из SD + перезагружает ПК. Результат записывается в JSON-файл (`/tmp/drova-status-<host>.json`) и отображается в UI:

| Иконка | Значение |
|---|---|
| `—` серый | Диагностика не запускалась с момента последнего старта воркера |
| `⊙` синий | Пропущена: при старте обнаружена активная Drova-сессия (это нормально) |
| `⚡` красный | Прервана: ошибка или `RebootRequired` (ПК ещё не перезагрузился после установки ESME) |
| `✓ 28/28` зелёный | Все ограничения применены и верифицированы, патчи без ошибок |
| `⚠ N/28` жёлтый | Часть ограничений не применилась или патч упал с ошибкой |

Наведите курсор на иконку — tooltip покажет: время запуска, список незакрытых ключей реестра, имена упавших патчей.

> **Важно:** Значение `—` после перезапуска воркера (например, через кнопку «Стоп → Старт» в UI) — штатная ситуация. Диагностика запустится автоматически, и иконка обновится через несколько минут.
...
| Кнопка | Действие |
|---|---|
| **Старт** | Запускает дочерний процесс воркера для этого хоста |
| **Стоп** | Останавливает воркер (graceful `SIGTERM`), записывает `"enabled": false` в `hosts.json` |
| **↺** (Перезагрузить) | Отправляет `shutdown /r /t 0 /f` на Windows ПК через SSH (с подтверждением) |
| **⏻** (Выключить) | Отправляет `shutdown /s /t 0 /f` на Windows ПК через SSH (с подтверждением) |
| **✕** (Удалить) | Останавливает воркер и удаляет хост из `hosts.json` (с подтверждением) |

> **Кнопки ↺ и ⏻** работают независимо от того, запущен ли воркер — они напрямую соединяются с Windows ПК по SSH. После перезагрузки воркер автоматически переподключится.

---

#### Фоновый SSH-зонд

Сервис `drova_web` параллельно с управлением воркерами выполняет фоновые SSH-проверки каждые 30 секунд (первый прогон через 15 секунд после старта). Для каждого хоста зонд:

1. Пробует SSH-подключение с текущими учётными данными → обновляет **SSH**
2. Выполняет `ShadowDefenderCLI /list` → обновляет **SD**
3. Делает `reg query` для всех ~28 ключей `PatchWindowsSettings` → обновляет **Огр.**
4. Читает `auth_token` из реестра, опрашивает Drova API → обновляет **Сессия**

Зонд полностью read-only — он ничего не меняет на Windows ПК. Каждый хост зондируется независимо с таймаутом 90 секунд.

---

#### REST API

Веб-сервис предоставляет REST API (все запросы требуют Basic Auth):

| Метод | Endpoint | Описание |
|---|---|---|
| `GET` | `/api/hosts` | Список хостов с полным статусом (воркер + зонд + стартовая диагностика) |
| `POST` | `/api/hosts/{host}/start` | Запустить воркер |
| `POST` | `/api/hosts/{host}/stop` | Остановить воркер |
| `POST` | `/api/hosts/{host}/reboot` | Перезагрузить Windows ПК |
| `POST` | `/api/hosts/{host}/shutdown` | Выключить Windows ПК |
| `PUT` | `/api/hosts` | Добавить хост (тело: `{"host":"...", "login":"...", "password":"..."}`) |
| `DELETE` | `/api/hosts/{host}` | Удалить хост |

**Пример ответа `GET /api/hosts`:**
...
`diag` — данные последнего SSH-зонда; `worker_diag` — данные последней стартовой диагностики.
...
- При старте `drova_web` читает `hosts.json` и **автоматически запускает все хосты с `enabled: true`**
- Хосты, остановленные через UI (кнопка «Стоп»), сохраняются в `hosts.json` как `"enabled": false` и **не запускаются** автоматически
- Если воркер падает в процессе работы, `drova_web` обнаруживает это через ~10 секунд и перезапускает его
- Файлы стартовой диагностики (`/tmp/drova-status-*.json`) **не очищаются** при перезапуске сервиса — в UI будет показана диагностика предыдущей сессии до тех пор, пока воркер не запустится и не обновит файл
...
#### `test_common.py` — команды и диагностика

| Тест | Что проверяет |
|---|---|
| `test_parse_PSExec` | Парсинг stderr PsExec (EN и RU Windows) |
| `test_parse_RegQueryEsme` | Парсинг `auth_token` из реестра |
| `test_qwinsta_parse_active_session_*` | `QWinSta` — EN/RU, bytes/str, нет сессии |
| `test_parse_all_auth_codes` | Несколько `server_id` в реестре |
| `test_windows_shutdown_*` | `WindowsShutdown` — ключи `/r`, `/s`, `/f` |
| `test_write_status_*` | `GamePCDiagnostic._write_status`: JSON-содержимое, no-op без env, skipped-случай |

#### `test_web_manager.py` — WorkerManager

| Группа | Что проверяет |
|---|---|
| `TestConfigLoad` | Загрузка config.json, defaults, enabled, ошибочный JSON |
| `TestConfigSave` | Сохранение, атомарная запись, корректность defaults |
| `TestStartWorker` | Запуск процесса, env-переменные (HOST/LOGIN/PASSWORD/`DROVA_STATUS_FILE`), идемпотентность |
| `TestStopWorker` | Graceful stop, SIGKILL по таймауту, `enabled: false` в конфиге |
| `TestAddHost` | Добавление хоста, дубликат → ValueError |
| `TestRemoveHost` | Удаление хоста, неизвестный → ValueError |
| `TestMonitorLoop` | Перезапуск упавшего воркера через ~10 сек |
| `TestGetStatus` | Статусы `running/stopped/disabled/error`, PID, exit_code |
| `TestWorkerDiag` | `_status_file_path` (детерминизм, уникальность), `_load_worker_diag` (чтение файла, битый JSON), `get_status` включает `worker_diag`, `start_worker` передаёт `DROVA_STATUS_FILE` в env |
| `TestConfigResilience` | Атомарная запись — оригинал не портится при ошибке `json.dump` |

#### `test_web_server.py` — HTTP API

| Группа | Что проверяет |
|---|---|
| `TestBasicAuth` | 401 без заголовка, 401 при неверном пароле, 200 при верном |
| `TestGetHosts` | GET `/api/hosts` — статус 200, JSON-массив, поля `host/status/pid` |
| `TestGetHostsDiag` | GET `/api/hosts` — поле `diag` с SSH/SD/restrictions/session, null-значения |
| `TestStartStop` | POST `/start` и `/stop` — 200, вызов методов manager, 404 для неизвестного хоста |
| `TestRebootHost` | POST `/reboot` — 200, вызов `reboot_host`, 404, требует Auth |
| `TestShutdownHost` | POST `/shutdown` — 200, вызов `shutdown_host`, 404, требует Auth |
| `TestAddHost` | PUT `/api/hosts` — 200, 409 дубликат, 400 пустой хост, пустой логин → None |
| `TestDeleteHost` | DELETE `/api/hosts/{host}` — 200, 404 |
...
4. Результат записывается в `/tmp/drova-status-<host>.json` и отображается в колонке **Диагн.↑** веб-UI
...
Если используется `drova_web`, итог диагностики можно посмотреть в UI (колонка **Диагн.↑**) без доступа к логам. Для подробностей (список недостающих ключей, упавшие патчи) наведите курсор на иконку.
