...
Раздел описывает скрипты для разовых операций: снятие застрявших ограничений и удаление Shadow Defender с группы машин.

### 9.1 Когда требуется вмешательство

| Ситуация | Что происходит | Инструмент |
|----------|---------------|------------|
| Клиент убит принудительно (`kill`, сбой питания) и Shadow Defender уже удалён или был с другим паролем | Реестровые ограничения остаются активными (заблокированы диспетчер задач, powershell, regedit и т.д.) | `--remove-restrictions` |
| Shadow Defender деинсталлирован вручную, но его файлы остались (`C:\Program Files\Shadow Defender`) | SD не работает, клиент не может ввести/выйти из режима снимка | `--uninstall-sd` |
