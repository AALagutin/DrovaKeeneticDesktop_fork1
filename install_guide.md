...
- [9. Обслуживание и аварийное восстановление](#9-обслуживание-и-аварийное-восстановление)
...
# Проверить статус Shadow Defender на всех машинах (только чтение, ничего не меняет):
DROVA_CONFIG=... poetry run python scripts/deploy.py --check-sd

# Установить SD только там, где он ещё не установлен (уже настроенные пропускаются):
DROVA_CONFIG=... poetry run python scripts/deploy.py \
    --sd-installer "D:\Setup\ShadowDefender_Setup.exe"
```
...
## 9. Обслуживание и аварийное восстановление

Раздел описывает механизм автоматической самоочистки клиента и скрипты для ситуаций, когда автоматика не сработала.

### 9.1 Автоматическая самоочистка (встроена в клиент)

При каждом подключении к ПК в режиме ожидания (нет активной сессии) клиент автоматически проверяет, не остались ли застрявшие ограничения (`DisableTaskMgr` в реестре HKCU). Если ограничения обнаружены — они снимаются через SSH без перезагрузки:

```
WARNING Worker[GAME12] Stuck restrictions detected (no active session) — cleaning up
INFO    Worker[GAME12] Removing stuck restrictions...
INFO    Worker[GAME12] Stuck restrictions removed successfully.
```

Это покрывает типовой сценарий: клиент упал → ПК перезагрузился → клиент переподключился → сам почистил реестр перед следующей сессией.

**Ручное вмешательство требуется** только когда авто-очистка не может отработать:

| Ситуация | Причина, по которой авто-очистка не помогает | Инструмент |
|----------|---------------------------------------------|------------|
| ПК не перезагружался, пользователь застрял с активными ограничениями прямо сейчас | Клиент видит ПК как «в сессии» или SSH недоступен | `--remove-restrictions` |
| Shadow Defender деинсталлирован вручную, но его файлы остались (`C:\Program Files\Shadow Defender`) | SD не работает, клиент не может войти/выйти из режима снимка | `--uninstall-sd` |
| Требуется полностью переустановить SD с новым паролем | Старая версия мешает установке новой | `--uninstall-sd`, затем ручная установка |

---

### 9.2 Снять застрявшие ограничения (`--remove-restrictions`)

**Что делает `scripts/remove_restrictions.ps1`:**

- Находит все интерактивные пользовательские сессии в `HKEY_USERS` (работает корректно при запуске от SYSTEM через PsExec)
- Снимает все реестровые ограничения, наложенные клиентом:
  - `DisableCMD`, `DisableTaskMgr`, `DisableGpedit`
  - Блокировку VBScript
  - Ограничения кнопок питания и выхода (`NoClose`, `NoLogoff` и др.)
  - `DisallowRun` с полным списком заблокированных приложений (regedit, powershell, anydesk и т.д.)
  - Ограничение MMC
  - `HideFastUserSwitching` (HKLM)
- Удаляет правила брандмауэра `Block SMB Out` и `Block NetBIOS Out`
- Восстанавливает Network Discovery (группа + службы FDResPub, SSDPSRV, fdPHost)
- Запускает `gpupdate /force` для немедленного применения

```bash
DROVA_CONFIG=/opt/drova-desktop/config.json \
    poetry run python scripts/deploy.py --remove-restrictions

# Или только на конкретных машинах:
DROVA_CONFIG=... poetry run python scripts/deploy.py \
    --remove-restrictions \
    --hosts 192.168.1.12,192.168.1.16,192.168.1.17
```

> **После выполнения:** перезапустите Explorer на ПК (`taskkill /f /im explorer.exe && start explorer.exe`) или перезайдите в учётную запись — некоторые ограничения снимаются только при новом входе.

---

### 9.3 Удалить Shadow Defender удалённо (`--uninstall-sd`)

**Что делает `scripts/uninstall_sd.ps1` (три попытки по приоритету):**

1. **NSIS-деинсталлятор** — ищет `unins*.exe` в папке SD, запускает с флагом `/S`
2. **WMI-деинсталляция** — ищет запись `Win32_Product` по имени "Shadow Defender"
3. **Ручная принудительная очистка** (если 1 и 2 не сработали — типично для вручную удалённых или нестандартно установленных версий):
   - Kill всех процессов `Shadow*`, `ShadowSrv*`, `CmdTool*`
   - `sc.exe delete` всех сервисов через WMI (находит даже незарегистрированные драйверы)
   - Удаление ключей реестра Uninstall (64-bit и WOW6432Node)
   - Удаление папки `C:\Program Files\Shadow Defender` (с `takeown` + `icacls` если нужно)

```bash
DROVA_CONFIG=/opt/drova-desktop/config.json \
    poetry run python scripts/deploy.py --uninstall-sd

# Только проблемные машины:
DROVA_CONFIG=... poetry run python scripts/deploy.py \
    --uninstall-sd \
    --hosts 192.168.1.12,192.168.1.16,192.168.1.17
```

> **После выполнения:** необходим **перезагрузка** каждого ПК для выгрузки драйвера ядра. Файлы SD могут оставаться до ребута — это нормально.

---

### 9.4 Проверить статус SD на всех машинах (`--check-sd`)

**Только чтение — никаких изменений на машинах не производится.**

Запускает `check_sd.ps1` на всех хостах параллельно и показывает статус по каждому:

| Статус | Значение | `[OK]`/`[FAIL]` |
|--------|----------|----------------|
| `not installed` | SD не установлен | `[OK]` — безопасно запускать `--sd-installer` |
| `installed / service running / password OK` | SD работает, пароль совпадает | `[OK]` — ничего делать не нужно |
| `installed / service running / no password configured` | SD работает без пароля | `[OK]` — нужно выставить пароль вручную в SD GUI |
| `orphaned files … run --uninstall-sd` | Файлы есть, но CmdTool/сервис отсутствуют | `[FAIL]` — нужен `--uninstall-sd` |
| `installed / service … not running — reboot required` | SD установлен, но драйвер не активен | `[FAIL]` — нужна перезагрузка |
| `WRONG PASSWORD` | SD работает, пароль в конфиге не совпадает | `[FAIL]` — обновить `shadow_defender_password` в `config.json` |
...
Пример вывода:
```
Checking SD status on 14 host(s) with parallelism=10
============================================================
[GAME12 / 192.168.1.12] Checking SD status...
[GAME13 / 192.168.1.13] Checking SD status...
...
============================================================
SUMMARY
============================================================
  [OK  ]  GAME12         192.168.1.12
            SD: not installed
  [OK  ]  GAME13         192.168.1.13
            SD: installed / service running / password OK
  [FAIL]  GAME16         192.168.1.16
            SD: installed / service running / WRONG PASSWORD (CmdTool exit 1)
         ERROR: Password mismatch — update shadow_defender_password in config.json
  [FAIL]  GAME17         192.168.1.17
            SD: orphaned files in 'C:\Program Files\Shadow Defender' (no CmdTool.exe) — run --uninstall-sd

  11/14 hosts checked successfully.
```

---

### 9.5 Полный порядок восстановления после инцидента

Сценарий: клиент был убит принудительно на нескольких ПК, SD деинсталлирован вручную с другим паролем, на части машин активны ограничения.

```
Шаг 0: Выяснить картину по SD на всех машинах
──────────────────────────────────────────────────────────────
poetry run python scripts/deploy.py --check-sd
# [FAIL] WRONG PASSWORD  → обновить пароль в config.json (шаг 5)
# [FAIL] orphaned files  → нужен --uninstall-sd (шаг 2)
# [FAIL] reboot required → перезагрузить (шаг 3)

Шаг 1: Снять ограничения (мгновенно, без ребута)
──────────────────────────────────────────────────────────────
poetry run python scripts/deploy.py --remove-restrictions \
  --hosts <список проблемных IP>

Шаг 2: Снести остатки SD (если папка ещё есть)
──────────────────────────────────────────────────────────────
poetry run python scripts/deploy.py --uninstall-sd \
  --hosts <список проблемных IP>

Шаг 3: Перезагрузить ПК вручную или через SSH
──────────────────────────────────────────────────────────────
for ip in 192.168.1.12 192.168.1.16 ...; do
    ssh Administrator@$ip "shutdown /r /f /t 0"
done

Шаг 4: Установить новый SD вручную на каждый ПК
──────────────────────────────────────────────────────────────
(см. п. 2.2)
Установить пароль, поставить Need password when committing.

Шаг 5: Обновить пароль в конфиге и проверить
──────────────────────────────────────────────────────────────
nano /opt/drova-desktop/config.json  # обновить shadow_defender_password
poetry run python scripts/deploy.py  # прогнать setup для проверки CmdTool
```

---

### 9.6 Таблица ошибок скриптов обслуживания

| Вывод | Причина | Решение |
|-------|---------|---------|
| `No interactive user sessions found in HKU` | Пользователь не залогинен на ПК | Залогиньтесь вручную, затем повторите |
| `Directory still present — a driver file may be locked` | Драйвер SD загружен в память | Перезагрузите ПК и повторите `--uninstall-sd` |
| `ERROR: cannot find uninstaller or WMI product entry` | (устарело) Скрипт теперь использует ручную очистку в этом случае | Обновите ветку (`git pull`) |
| `[FAIL] Execution failed: timed out` | WMI-запрос Win32_Product завис (>10 мин) | Запустите только `--hosts` с одной машиной для диагностики |
| `gpupdate` не применяет изменения | Explorer ещё не перезапущен | Выйдите и зайдите в учётную запись Windows или сделайте `taskkill /f /im explorer.exe` |
